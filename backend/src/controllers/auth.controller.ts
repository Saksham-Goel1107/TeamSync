...
export const googleLoginCallback = asyncHandler(
  async (req: Request, res: Response) => {
    try {
      ...
      if (!currentWorkspace) {
        console.error('No workspace found for user after Google authentication');
        return res.redirect(
          `${config.FRONTEND_GOOGLE_CALLBACK_URL}?status=failure&reason=no_workspace`
        );
      }

      // With cookie-session, changes are saved automatically
      return res.redirect(
      ...
      return res.redirect(
        `${config.FRONTEND_GOOGLE_CALLBACK_URL}?status=failure&reason=server_error`
      );
    }
  }
);
