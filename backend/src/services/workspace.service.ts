...
export const regenerateInviteCodeService = async (
  ...
  if (!member || (member.role.name !== Roles.OWNER && member.role.name !== Roles.ADMIN)) {
    throw new BadRequestException(
      "You don't have permission to regenerate invite code"
    );
  }

  // Deactivate old invite code first
  ...
  return {
    workspace: {
      ...workspace.toObject(),
      inviteLink: `${process.env.FRONTEND_ORIGIN}/invite/workspace/${workspace.inviteCode}/join`
    },
  };
};
